...
//                .replace("<ph ", "\n\t\t\t\t<ph ");
    ...
    public static void sendOutput(String server,String jobId, Document doc){
        try{
            ...
            re.setParameter("data", new XMLOutputter(org.jdom.output.Format.getPrettyFormat().setTextMode(TextMode.NORMALIZE).setIndent("    ").setOmitEncoding(false)).outputString(doc));
            // In line above, Eoin changed setOmitEncoding(true) to setOmitEncoding(false).

            // Eoin adding setEncoding to see if that helps.
            //re.setParameter("data", new XMLOutputter(org.jdom.output.Format.getPrettyFormat().setTextMode(TextMode.NORMALIZE).setIndent("    ").setEncoding("UTF-8").setOmitEncoding(true)).outputString(doc));
